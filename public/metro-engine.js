...
function boot(){
  ...
  function buildB(MA){
    ...
    for(var pi=0;pi<pages.length;pi++){
      var pg=pages[pi], yoff=pageYoff[pi], xoff=pageXoff[pi], scale=pageScale[pi], bxs=pg.bxs, cxs=pg.cxs;
  ...
  function start(){
    ...
    doCountIn(function(){
      ...
      if(st.loopOn){ playLoop(); } else if(seq){ playSequence(seq); } else { playLoop(); }
    });
  }
  ...
  function playSequence(seq){
    ...
    function playSeg(){
      if(!st.playing) return;
      if(segI>=seq.length){ finishAt(0); info.textContent='sequence done'; return; }
      var rng=barRangeToIdx(seq[segI][0], seq[segI][1]);
      var s0=rng[0], s1=rng[1];
      if(s0<0){ segI++; return playSeg(); }
      var spb=60/st.bpm, t0=performance.now(), lastBi=0, nBeats=s1-s0+1;
      st.iSeq=s0; lastRowY=-1; place(s0); click(B[s0][5]===1);
      info.textContent='seq '+(segI+1)+'/'+seq.length+' (m'+seq[segI][0]+'-'+seq[segI][1]+')';
      function fr(ts){ if(!st.playing) return;
        var bi=Math.floor((ts-t0)/1000/spb);
  ...
  function finishAt(s0){ st.playing=false; if(st.raf)cancelAnimationFrame(st.raf);
    setPlayBtn(false); st.iSeq=s0; lastRowY=-1; place(s0); }
  function stop(){ st.playing=false; if(st.raf)cancelAnimationFrame(st.raf);
    setPlayBtn(false); info.textContent='stopped'; }
  ...
  function __metroKeydown(ev){
    ...
    if(!B.length) return;
    var k=ev.key, moved=false;
    if(k==='ArrowRight'){ if(st.iSeq<B.length-1){ st.iSeq++; moved=true; } }
    else if(k==='ArrowLeft'){ if(st.iSeq>0){ st.iSeq--; moved=true; } }
    else if(k==='ArrowDown'){ var m=B[st.iSeq][4]; for(var i=st.iSeq+1;i<B.length;i++){ if(B[i][4]===m+1){ st.iSeq=i; moved=true; break; } } }
    else if(k==='ArrowUp'){ var f=barFirst(st.iSeq); if(f>0){ st.iSeq=barFirst(f-1); moved=true; } else { st.iSeq=0; moved=true; } }
    ...
    if(st.playing){ st.playing=false; if(st.raf)cancelAnimationFrame(st.raf); setPlayBtn(false); }
    lastRowY=-1; place(st.iSeq); click(B[st.iSeq][5]===1);
  ...
  p.innerHTML=
    ...
    '<div id=sgBody>'+
    '<div style="margin-bottom:12px"><div style="display:flex;align-items:center;gap:10px"><span style="width:34px;color:'+T.sub+'">BPM</span><input id=sgBpm type=range min=20 max=300 value=66 style="flex:1;accent-color:'+T.acc+'"><b id=sgBpmV style="font-size:26px;font-weight:800;color:'+T.acc+';min-width:46px;text-align:right;line-height:1">66</b></div></div>'+
    ...
    '<div style="font-size:10px;color:'+T.sub+';text-align:center" id=sgHint>L/R: beat \u00b7 U/D: bar \u00b7 space: play</div>'+
  ...
  // ===== 面板事件 =====
  document.getElementById('sgBpm').oninput=function(){ st.bpm=+this.value; document.getElementById('sgBpmV').textContent=this.value; };
  ...
  (function(){
    ...
    if(c.bpm!=null){ st.bpm=c.bpm; setV('sgBpm',c.bpm); var bv=document.getElementById('sgBpmV'); if(bv)bv.textContent=c.bpm; }
