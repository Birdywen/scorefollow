 ...
 * 用法: node scripts/nb-veto-check.mjs [score]
  ...
  for (const gpi of gt.pages) {
    ...
    pairs.forEach((oi, gi) => {
      ...
      for (const w of want) {
        ...
        if (found >= 0) {
          used[found] = true;
          const o = op.systems[oi];
          const dx = Math.round(internal[found]);
          let vetoNb = [];
          for (let ddx = -2; ddx <= 2; ddx++) {
            if (legacy.barColumnVetoed(oi, dx + ddx, o.y1, o.y2)) vetoNb.push(ddx);
          }
          if (vetoNb.length) console.log(`${score} p${gpi.page} sys${oi + 1},${dx},nbveto=[${vetoNb.join(",")}]`);
        }
      }
