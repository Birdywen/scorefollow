 ...
 */
export function countPixFromBuffer(w: number, h: number, data: Uint8ClampedArray | number[], seln: number): CountPixResult { var c: any, e: any; var f: any = w; var g = 4 * f; var k = h; var a: any = data; var l = 0; var p: any = []; var n = 3 * g / 4, stride = g; legacyOpt.eerst && (n = 0, stride = g / 4); for (e = 0; e < k; e++) { var m = 0; for (c = l + n; c < l + stride; c += 4)m += a[c], m += a[c + 1], m += a[c + 2]; c = m / (3 * (stride - n)); p.push(c); l += g } f = drawRes(p, f, k); lastRowGroups.length = 0; for (const gg of (f as number[][])) lastRowGroups.push((gg as number[]).slice()); for (f = countVsys(f, g, a); f.length && skipnV > 0;)f.shift(), --skipnV; for (; f.length && skipnV < 0;)f.pop(), ++skipnV; seln && (f = f.slice(seln - 1, seln)); const foundBars = findBarLines(f, g, a); return { cxs: f, bxs: foundBars } }
