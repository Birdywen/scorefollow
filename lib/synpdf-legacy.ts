...
export function barColumnVetoed(sysIdx: number, x: number, rw0: number, rt0: number): boolean {
  ...
  return ((Math.max(sf0.topBlob, sf0.botBlob) >= 4 && sf0.runRatio >= 0.8 && sf0.midWidth <= 5) ||
    ...
    // rule4 过宽否决的双线对豁免(Serenade p4s1: thin+thin 双线 mid=9,
    // 小印张下复合体必宽; 孪生强直(0.95+, 3-5px)即系出同源, 宽度由此而来)。
    // 豁免(run≥0.97)先行返回, 本 carve 只管 run<0.97 的半截双线;
    // 符干对尾巴(rel≥0.95 的极少数)仍有 rule1/2/6 兜底(头必在带内)。
    (sf0.runRatio >= 0.6 && sf0.midWidth >= 6 &&
      !(sf0.twinRel >= 0.95 && sf0.twinDist >= 3 && sf0.twinDist <= 5)) ||
...
/** 淡线抢救准入(Serenade p4s6-356: 强峰 rel 0.91 + 对比度一侧差 0.7 而死,
 * run 0.875 够不到 0.9 直通)。只认“成对线的半截”(twinRel≥0.9: 双线对/
 * 贴杆对的弱半边, 356 twin=4/0.957; 孤柱(twin 0)一律不救——suzuki 五符干
 * 全是孤柱) + “孤独干净竖线”: 端部无宽行(topB≤2)、下方无梁墨(beamB≤1)、
 * 带内无符头、上方无延伸(ext≤2)、不宽(mid≤5)。
 * Boek 三符干(323 topB=7/prox=2; 382/808 beamB=3/ext≥4)纵使有 twin 也过不了干净门。
 * 弱峰不走此门(弱+短=伪影高发区)。 */
export function barRescueClean(sysIdx: number, x: number, rw0: number, rt0: number): boolean {
  var f0 = barStemFeatures(sysIdx, x);
  if (!f0) return false;
  if (f0.twinRel < 0.9 || f0.twinDist < 3) return false;
  if (f0.topBlob > 2 || f0.beamBelow > 1 || f0.extAbove > 2 || f0.midWidth > 5) return false;
  var spC = Math.max(1, spatium);
  for (const sg of (f0.headSegs ?? [])) {
    if (sg.y < rt0 + 0.5 * spC && sg.y + sg.h > rw0 - 0.5 * spC) return false;
  }
  return true;
}
