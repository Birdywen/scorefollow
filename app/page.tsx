...
export default function ScoreFollowPage() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "f") { e.preventDefault(); setFilesOpen((v) => !v); return; }
      ...
      if (e.key === "Escape") { setHelpOpen(false); setAdvOpen(false); setPie(null); return; }
    ...
    }, [playing, doPlay, doPause, now, mediaURL, synbox, doTap, backupOne, adjustLast, correctMode, deleteSelectedBar, doUndo, doRedo, placeCursor, stepSystem, pageNum, numPages, renderPage, applyAdv, selectedBar, splitSelectedMeasure, mergeSelectedMeasure]);
  ...
  return (
    <main className={styles.page} data-fullscreen={fullScreen ? "true" : "false"} data-theme={darkTheme ? "dark" : "light"}>
      ...
      {chromeOpen && <header className={styles.topbar}>
        <span className={styles.tbLogo}><span className={styles.tbLogoMark} />ScoreFollow</span>
        <span className={styles.tbGroup}>
          <button className={styles.tbBtn} onClick={() => setChromeOpen(false)} title="Hide toolbar (T)">hide UI</button>
          <button className={styles.tbBtn} onClick={() => setFilesOpen((v) => !v)} title="Toggle file controls">files</button>
        ...
        <span className={styles.tbLabel}>speed <input type="range" min={0.1} max={4} step={0.05} value={speed} onChange={(e) => setSpeed(Number(e.target.value))} /> {speed.toFixed(2)}x</span>
        <span className={styles.tbSep} />
        <span className={styles.tbGroup}>
        ...
        <span className={styles.tbSep} />
        <span className={styles.tbLabel}>page
      ...
      {chromeOpen && filesOpen && <section className={styles.topbar} aria-label="file-controls">
        ...
        <label className="pill"><input type="checkbox" checked={loopB > loopA} onChange={(e) => {
          if (e.target.checked) wijzerRef.current.setLoop(loopA || 0, loopB || Math.max(loopA + 4, 4));
          else wijzerRef.current.setLoop(0, 0);
        }} /> loop</label>
        <label className="pill">A–B <input type="number" min={0} step={0.5} value={loopA} onChange={(e) => { const v = Number(e.target.value); setLoopA(v); wijzerRef.current.setLoop(v, loopB); }} style={{ width: 64 }} />
      ...
      {helpOpen && <section className={styles.advpanel} role="dialog" aria-label="help">
        ...
        <div>F file controls · H help · L line cursor · M menu · Esc close</div>
